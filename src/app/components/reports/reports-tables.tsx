...
const ReportsTables = () => {
	...
	if (!isLoading && !isError && items?.length === 0) {
		content = <ErrorMsg msg="No Report Found" />;
	}
				...
				<div className="overflow-scroll 2xl:overflow-visible">
					<div className="tp-search-box flex items-center justify-between px-8 py-8">
						...
						<div className="search-input relative">
							<input onChange={handleSearchProduct} className="input h-[44px] w-full pl-14" type="text" placeholder="Search by name"/>
							<button className="absolute top-1/2 left-5 translate-y-[-50%] hover:text-theme">
								<Search />
							</button>
						</div>
						<button onClick={handleDownloadCSV} className="bg-green-500 text-white px-4 py-2 rounded-md">
							Download CSV
						</button>
					</div>
