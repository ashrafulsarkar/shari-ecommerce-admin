...
import { useDeleteAreaMutation } from "@/redux/area/areaApi";
...
const AreaEditDelete = ({ id }: IPropType) => {
  const [showEdit, setShowEdit] = useState<boolean>(false);
  const [showDelete, setShowDelete] = useState<boolean>(false);
  const router = useRouter()
  const [deleteArea, { data: delData, error: delErr }] =
      ...
      if (result.isConfirmed) {
        try {
          ...
          if ("error" in res) {
            ...
            Swal.fire("Deleted!", `Your Area has been deleted.`, "success");
            router.push('/area')
          }
        } catch (error) {
